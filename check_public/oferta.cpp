...
double find_min_price(const unsigned int products[], const unsigned int nr_products) {
    ...
    const double product_and_pair = products[0] + apply_pair_discount(products[1], products[2]);
    const double pair_and_product = apply_pair_discount(products[0], products[1]) + products[2];
    const double first_triple = apply_triple_discount(products[0], products[1], products[2]);
    ...
    for (unsigned int i = 3; i < nr_products; i++) {
        ...
        prev_and_pair = dp[i - 2] + apply_pair_discount(products[i - 1], products[i]);
        prev_and_triple = dp[i - 3] + apply_triple_discount(products[i - 2], products[i - 1], products[i]);
...
int main()
{
    ...
    if (K > 1) {
        ...
        out << fixed << setprecision(1) << find_min_price(products, nr_products);
    }
