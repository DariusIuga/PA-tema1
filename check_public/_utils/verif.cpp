...
// Checks if a part of an answer is acceptable, by comparing it with the
// reference.
...
#else
            if (use_double) {
                sprintf(c, "WA %.1lf vs %.1lf", answer_out.value.db,
                        answer_ref.value.db);
            } else {
                sprintf(c, "WA %lld vs %lld", answer_out.value.ll,
                        answer_ref.value.ll);
            }
#endif
